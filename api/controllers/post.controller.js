...
module.exports.create = asyncHandler(async (req, res) => {
  try {
    ...
    res.status(201);
    res.json({status: 201, message: "Post created successfully", data: newPost });
  } catch (error) {
    console.error(error);
...
module.exports.getPostById = asyncHandler(async (req, res) => {
  try {
    ...
    if (!channelId) {
      ...
      return res.status(404).json({ message: "Channel ID not found in post data" });
    }
    ...
    if (!userId) {
      ...
      return res.status(404).json({ message: "User ID not found in channel data" });
    }
...
module.exports.upvotePost = asyncHandler(async (req, res) => {
  try {
    ...
    const isAlreadyVoted = await Vote.findOne({votingUserId: userId, postId : postId});
    if(isAlreadyVoted && isAlreadyVoted.voteType === true) {
      ...
      await newVote.save();
      return res.status(201).json({status : 201, message: "Success", data: newVote });
    }
...
module.exports.downvotePost = asyncHandler(async (req, res) => {
  try {
    ...
    if(isAlreadyVoted && isAlreadyVoted.voteType === false) {
      return res.status(200).json({status: 400, message: "Already downvoted"});
    }
...
module.exports.voting = asyncHandler(async (req, res) => {
  try {
    ...
    if (votingData && votingData.length > 0) {
      return res.status(200).json({ status: 200, message: "Success", data: votingData });
    } else {
      return res.status(200).json({ status: 404, message: "No voting data found" });
    }
  } catch (error) {
    console.error(error);
    return res.status(500).json({ error: "Internal Server Error" });
...
module.exports.bookmarkPost = asyncHandler(async (req, res) => {
  try {
    ...
    if (type !== 'post' && type !== 'comment') {
      return res.status(400).json({ error: "Invalid type parameter. Must be 'post' or 'comment'" });
    }

    const filter = type === 'post' ? { userId, postId: _id } : { userId, postCommentId: _id };
    const isBookmark = await Bookmark.findOne(filter);

    if (isBookmark) {
      // If the bookmark exists, remove it
      await Bookmark.deleteOne(filter);
      return res.status(200).json({ message: "Removed from bookmark list" });
    }

    // If the bookmark does not exist, add it
    const bookmark = type === 'post' ? { userId, postId: _id } : { userId, postCommentId: _id };
    const bookmarkPost = new Bookmark(bookmark);
    await bookmarkPost.save();

    return res.status(201).json({status: 201,  message: "Added to bookmark", data: bookmarkPost });
  } catch (error) {
    console.error(error);
    return res.status(500).json({ error: "Internal Server Error" });
  }
  ...
  try {
    ...
    const bookmarks = await Bookmark.aggregate([
      ...
      },
      {
        $lookup: {
          from: 'posts',
          let: { postCommentPostId: { $arrayElemAt: ['$postComment.postId', 0] } },
          pipeline: [
      ...
      },
      {
        $lookup: {
          from: 'channels',
          let: { postCommentChannelId: { $arrayElemAt: ['$postCommentPost.channelId', 0] } },
          pipeline: [
    ...
    return res.status(200).json({status: 200, message: 'Success', data: bookmarks });
  } catch (error) {
    console.error(error);
    return res.status(500).json({ error: 'Internal Server Error' });
  }
...
module.exports.removeBookmarkPost = asyncHandler(async (req, res) => {
  try {
    ...
    const isAlreadyBookmarked = await Bookmark.findOne({ userId, [searchField]: _id });
...
module.exports.getAllCommentReplies = asyncHandler(async (req, res) => {
  ...
  try {
    const replies = await PostCommentReply.find({ postCommentId })
      .populate({
        path: 'postCommentId',
        populate: {
          path: 'userId',
...
module.exports.upvoteComment = asyncHandler(async (req, res) => {
  try {
    ...
    const isAlreadyVoted = await CommentVote.findOne({votingUserId: userId, postCommentId : postCommentId});
    if(isAlreadyVoted && isAlreadyVoted.voteType === true) {
      return res.status(200).json({status: 400, message: "Already upvoted"});
    }
    else if(isAlreadyVoted && isAlreadyVoted.voteType === false) {
      ...
      await newVote.save();
      return res.status(200).json({status : 201, message: "Success", data: newVote });
    }
...
module.exports.commentVoting = asyncHandler(async (req, res) => {
  try {
    ...
    if (votingData && votingData.length > 0) {
      ...
      return res.status(200).json({ status: 404, message: "No voting data found" , data: []});
    }
  } catch (error) {
    console.error(error);
    return res.status(500).json({ error: "Internal Server Error" });
...
module.exports.getAllUnreadPost = asyncHandler(async (req, res) => {
  try {
    ...
    if (!channelId || !userId) {
      return res.status(200).json({status: 400,  message: "channelId and userId are required" });
    }
...
module.exports.deleteUnreadPost = asyncHandler(async (req, res) => {
  try {
    ...
    if (result.deletedCount === 0) {
      return res.status(200).json({status : 404, message: "Unread post not found" });
    }

    return res.status(200).json({status: 200,  message: "Unread post deleted successfully" });
  } catch(error) {
    console.error(error);
    return res.status(500).json({ error: "Internal Server Error" });
  }
