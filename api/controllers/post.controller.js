...
module.exports.getAllPost = asyncHandler(async (req, res) => {
  try {
    ...
    const postData = await Post.find()
      ...
      const postsWithBookmarkStatus = await Promise.all(postData.map(async (post) => {
...
module.exports.getPostById = asyncHandler(async (req, res) => {
  try {
    ...
    const bookmark = await Bookmark.findOne({ userId:mainuserId, postId: postId });
...
module.exports.addBookmark = asyncHandler(async (req, res) => {
  try {
    ...
    if (isBookmark) {
      ...
      return res.status(200).json({ message: "Removed from bookmark list",data:isBookmark });
    }
...
try{
  const userId = req.user._id;
  const postBookmarks = await Bookmark.find({ userId, postId: { $exists: true } }).sort({ createdAt: -1 }).populate({path:'postId',populate: {
        path: 'channelId',
        model: 'Channel'
      }});
...
try{
  const userId = req.user._id;
  const commentBookmarks = await Bookmark.find({ userId, postCommentId: { $exists: true } }).sort({ createdAt: -1 }).populate(
    {path:'postCommentId',populate: {
      path: 'userId',
      model: 'User'
    }}
  );
...
module.exports.getAllComment = asyncHandler(async (req, res) => {
  try {
    ...
    const votes = await CommentVote.find({ postCommentId: { $in: commentIds } })
    // Merge votes into comments
    const commentsWithVotes = comments.map(comment => {
      const commentVotes = votes.filter(vote => vote.postCommentId.equals(comment._id));
      const trueCount = commentVotes.filter(vote => vote.voteType === true).length;
      const falseCount = commentVotes.filter(vote => vote.voteType === false).length;
    ...
    const commentsWithBookmarkStatus = await Promise.all(commentsWithVotes.map(async (comment) => {
      const bookmark = await Bookmark.findOne({ userId, postCommentId: comment._id });
      return {
        ...comment,
        isBookmarked: !!bookmark
      };
    }));
    return res.status(200).json({status: 200, message: "Success", data: commentsWithBookmarkStatus });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal Server Error" });
  }
...
module.exports.getAllCommentReplies = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const replies = await PostCommentReply.find({ postCommentId:commentId })
      ...
      const replyVotes = await CommentVote.find({ postCommentId: { $in: replyIds } }).lean();

    const repliesWithVotes = replies.map(reply => {
      const replyUpvoteCount = replyVotes.filter(vote => vote.postCommentId.toString() === reply._id.toString() && vote.voteType).length;
      const replyDownvoteCount = replyVotes.filter(vote => vote.postCommentId.toString() === reply._id.toString() && !vote.voteType).length;
      return {
        ...reply,
...
const addRecentlyViewedPost = async (userId, postId) => {
  ...
  recentlyViewed.posts = recentlyViewed.posts.filter(id => id.toString() !== postId.toString());
...
module.exports.getRecentlyViewedPosts = async (req, res) => {
  try {
    const userId = req.user._id;
    const recentlyViewed = await RecentlyViewedPosts.findOne({ user: userId }).populate({
      path: 'posts',
      populate: {
        path: 'channelId',
        model: 'Channel'
    ...
    const postsWithBookmarkStatus = await Promise.all(recentlyViewed.posts.map(async (post) => {
      const bookmark = await Bookmark.findOne({ userId, postId: post._id });
      return {
        ...post.toObject(),
        isBookmarked: !!bookmark
